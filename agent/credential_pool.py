...
# ``PoolRotationMixin`` (fork-owned) leads the MRO: it overrides the rotation half of
# selection. Upstream's admin and model-cooldown mixins follow unchanged; no member of the
# three collides.
class CredentialPool(PoolRotationMixin, CredentialPoolAdminMixin, CredentialPoolModelCooldownMixin):
