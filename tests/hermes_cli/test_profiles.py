...
class TestProcessListerSeam:
    ...
    def test_no_inspector_answers_no_bound_backends(self, profile_env, monkeypatch):
        """``read() -> None`` is upstream's "psutil can't inspect anything": empty.

        The fork's refusal on that arm (``ProcessTableUnreadable`` /
        ``ProfileDeleteBlocked``) was retired 2026-09-24 — psutil is a pinned
        core dependency, so the arm is unreachable in a real install.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
    ...
    def test_a_readable_table_deletes_exactly_as_before(
        ...
        assert not profile_dir.is_dir()
        # ONE read for the whole delete: the stop step's scan.
        assert lister.reads == 1
