...
class TestWriteFileHandler:
    ...
    def test_writes_content(self, mock_get):
        ...
        # Same as TestPatchHandler below: the handler forwards the RESOLVED
        # absolute path, spelled with the platform's own separator.
...
class TestPatchHandler:
    ...
    def test_replace_mode_calls_patch_replace(self, mock_get):
        ...
        assert result["status"] == "ok"
        # The handler forwards the RESOLVED absolute path, and
        # ``_resolve_path_for_task`` normalizes with the platform's own
        # separator. Pinning the literal POSIX spelling pinned ``os.sep``
        # rather than the forwarding guarantee.
