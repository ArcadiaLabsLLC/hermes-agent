"""Fork-owned half of ``tests/cron/test_cron_profile_isolation.py``.

Upstream's ``test_cron_storage_anchors_at_profile_home`` drops its env pins with a
mid-test ``monkeypatch.undo()``, which unwinds the shared per-test MonkeyPatch (the
root conftest's hermetic pins included) and is red under the fork's
``_shared_monkeypatch_pin_tripwire``; it is a skip row in
``tests/_downstream/id_markers.py``. This is the same test with the pins in a
scoped ``pytest.MonkeyPatch.context()`` and its own ``_set_profile_env``, which
takes the scoped patcher.
"""
