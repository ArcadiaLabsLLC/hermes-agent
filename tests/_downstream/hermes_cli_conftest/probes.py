...
# suite-cost-centres-2026-09-24.md`): the import-time probes cost every
# hermes_cli process a `node --version` spawn (and a local-model port probe,
# since deleted with the tests it guarded) — ~2.5 s x 1,260 files.
@functools.cache
...
def _module_missing(name: str) -> bool:
    """One real import attempt per name per process (see :func:`_no_module`)."""
    try:
...
def _no_module(name: str):
    ...
    """
    probe = functools.partial(_module_missing, name)
    probe.__name__ = f"_no_module({name!r})"
    return probe
...
def _no_posix_mode_bits() -> bool:
    ...
    with _tempfile.TemporaryDirectory() as _tmp:
        ...
        return _stat.S_IMODE(os.stat(_probe_file).st_mode) != 0o600
