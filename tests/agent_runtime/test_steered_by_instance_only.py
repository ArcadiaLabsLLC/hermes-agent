...
def test_cli_repair_steering_dry_run_then_real(monkeypatch, capsys):
    ...
    code = persona_commands._cmd_persona_instance_repair_steering(_repair_args(iid, dry_run=True))
    ...
    code = persona_commands._cmd_persona_instance_repair_steering(_repair_args(iid))
    assert code == 0
...
def test_cli_repair_steering_requires_a_target_or_all(monkeypatch, capsys):
    ...
    monkeypatch.setattr(persona_commands, "load_agent_runtime_config", _assignment_config)
    code = persona_commands._cmd_persona_instance_repair_steering(_repair_args(None))
    assert code == 2
