...
def test_proxy_env_rejects_malformed_port(monkeypatch, key):
    ...
    # (?i): os.environ upper-cases variable names on Windows, so the message
    # names HTTP_PROXY where the parametrisation says http_proxy. The
    # guarantee is that it names THAT variable, not its casing.
