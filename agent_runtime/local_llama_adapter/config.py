"""The adapter's settings document: the one carrier upstream has no home for.

Upstream ``local_runtime`` configures an engine (``tag``/``backend``/``port``); it has no
per-model load knobs (row 9, owner-ruled KEEP), no per-model generation parameters (row 10),
no bring-your-own executable (GAP-PR-1) and no extra model roots (GAP-PR-2). Those four live
here, under the ``local_llama`` key of the Hermes root config, published crash-safely with a
journal so a guarded ``config.set`` receipt never disagrees with the file (row 13).
"""
...
def recover_journal(directory, store):
    """Finish or roll back a ``config.set`` publication a crash interrupted."""
...
def publish_journal(directory, store, before, after, revision):
    from utils import atomic_json_write
    path = directory / "config-journal.json"
    if path.exists():
