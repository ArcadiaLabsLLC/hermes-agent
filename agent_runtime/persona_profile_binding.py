...
# ``backfill_instance_profile_ids`` (B-4's one-time null-stamp migration, 138
# lines) was DELETED 2026-09-24 (lane FORK-CODE, plan
# ``downstream-god-file-refactor.md`` §4.1): no CLI verb, no RPC, only its own
# tests called it. Ledger cite: ``08-performance-and-debt-ledger.md``, the
# DEAD_CODE_AUDIT_PASS_2 §4 subjects. A null ``profile_id`` projection still
# resolves through its persona's binding; ``rebind_persona_profile`` stamps.
