...
guard). Two callers outside the CLI run a turn in-process through it — the
agent-chat tool's inline relay (``tools/agent_chat/send.py``) and the dispatch
delivery's forged turn (``agent_runtime/dispatch_delivery.py``, lane B3) — and
neither may import the CLI namespace: that is the runtime reaching UP.
...
def run_mission_chat_turn(args: Any) -> tuple[int, dict | None]:
    ...
    if handler is None:
        raise MissionChatDoorUnbound(
            "no mission-chat turn handler is bound in this process: the harness "
