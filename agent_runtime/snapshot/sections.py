...
from agent_runtime.snapshot.envelope import parity_envelope
...
class SnapshotFrameBuild:
    """One snapshot frame, built phase by phase in :data:`SECTIONS` order.

    ``_build_snapshot_in_runtime_scope`` used to be one 334-line body whose
    locals every later section read; they are the fields here, and each phase
    is a method. Every comment is the old body's, moved with its code.
    """

    def __init__(self, agent_store=None, event_log=None, prompt_skills_catalogs=None, *, session_db) -> None:
        self.prompt_skills_catalogs = prompt_skills_catalogs
        self.session_db = session_db
        self.build_started = time.perf_counter()
        # THE SOURCE POSITION IS CAPTURED HERE, BEFORE THE FIRST SECTION IS READ —
        # not in ``parity_envelope`` at the end of the build. Every section below
        ...
        self.build_start_position = events_position()
        self.sections_ms: dict[str, int] = {}
        ...
        self.event_log = event_log or CachedEventLog()

    def read_events(self) -> None:
        """Materialize the event log once, timed as ``events``."""
        ...
        with _timed_section(self.sections_ms, "events"):
            self.recent_events = self.event_log.tail(20)

    def resolve_scope(self) -> None:
        """Config, the runtime personas, the workspace/realm stores and active names, the skill resolver."""
        ...
        self.cfg = load_agent_runtime_config()
        ...
        self.agents = [agent for agent in self.agent_store.list_all() if is_runtime_persona(agent)]
        self.workspace_store = WorkspaceStore()
        self.realm_store = RealmStore()
        self.workspaces = self.workspace_store.list_all(include_archived=True)
        self.realms = self.realm_store.list_all(include_archived=True)
        ...
        self.active_workspace_name = next(
            (getattr(w, "name", None) for w in self.workspaces if getattr(w, "id", None) == self.workspace_store.active_id()),
            None,
        )
        self.active_realm_name = next(
            (getattr(r, "name", None) for r in self.realms if getattr(r, "id", None) == self.realm_store.active_id()),
            None,
        )
        self.build_context = _SNAPSHOT_BUILD_CONTEXT.get()
        self.skill_resolver = _SkillObservabilityResolver(
            root_registries=(
                self.build_context.skill_root_registries if self.build_context is not None else None
            )
        )

    def agents_readiness(self) -> None:
        """Readiness and tool visibility per persona, timed as ``agents_readiness`` (split receipt logged)."""
        ...
        self.readiness_split: dict[str, int] = {}
            ...
            with _timed_section(self.readiness_split, "tool_visibility_ms"):
                self.agent_summaries = [
                    _agent_summary(
                        ...
                        readiness=self.readiness_by_persona_id.get(
                            str(getattr(agent, "id", "") or "")
                        ),
                    )
                    for agent in self.agents
                ]
        _log_agents_readiness_split(self.readiness_split)
        self.available_personas = _available_persona_summary(self.agents)
        self.personas_by_id = {str(getattr(agent, "id", "") or ""): agent for agent in self.agents}

    def persona_roster(self) -> None:
        """The persona-instance roster (unconditional since S56) and the migration status."""
        ...
        self.instance_store = PersonaInstanceStore(event_log=self.event_log)
        self.persona_instances = self.instance_store.ensure_for_personas(self.agents)
        self.topology_persona_instances = self.persona_instances
        self.agent_summaries = [
            *self.agent_summaries,
            *active_persona_instance_agent_summaries(
    ...
    def prompt_observability(self) -> None:
        """The prompt-observability section, timed."""
        ...
        with _timed_section(self.sections_ms, "prompt_observability"):
            self.prompt_observability_section = snapshot_prompt_observability(
                personas=self.agents,
                persona_instances=self.persona_instances,
                session_db=self.session_db,
                daemon=None,
                realm=self.active_realm_name,
                workspace=self.active_workspace_name,
                active_workspace_id=self.workspace_store.active_id(),
                catalog_sink=self.prompt_skills_catalogs,
                skill_resolver=self.skill_resolver,
            )

    def boards_offices(self) -> None:
        """The board and office projections, timed together as ``boards_offices``."""

        with _timed_section(self.sections_ms, "boards_offices"):
            self.boards_projection = _boards_summary(BoardStore(event_log=self.event_log), self.workspaces)
            self.boards_section = _keyed(self.boards_projection.boards, "board_id")
            self.offices_projection = _offices_summary(OfficeStore(event_log=self.event_log), self.workspaces, self.realms)
    ...
    def frame(self) -> None:
        ...
        self.data = {
            ...
            "generated_at": now(),
            "summary": SnapshotSummary(persona_instances=len(self.topology_persona_instances)).as_dict(),
            ...
            "runtime_default": {
                "model": _safe_model_label(self.cfg.default_model),
                "provider": _safe_model_label(self.cfg.default_provider),
                "api_mode": self.cfg.default_api_mode,
                "model_source": getattr(self.cfg, "default_model_source", "unset"),
                "provider_source": getattr(self.cfg, "default_provider_source", "unset"),
            },
            "runtime_config": effective_config_summary(self.cfg),
            "migration": self.migration,
            "prompt_observability": self.prompt_observability_section,
            "repo_scopes": _repo_scopes_summary(),
            "workspaces": [
                workspace_summary(
                    item,
                    persona_instances=self.topology_persona_instances,
            ...
            "boards_unreadable": self.boards_projection.unreadable,
            ...
            "offices_unreadable": self.offices_projection.unreadable,
            ...
            "running_work": self.running_work_section,
            "active_workspace_id": self.workspace_store.active_id(),
            "active_realm_id": self.realm_store.active_id(),
            "agents": self.agent_summaries,
            "available_personas": self.available_personas,
            "runtime_paths_diagnostic": _runtime_paths_diagnostic(self.available_personas),
        }

    def persona_rows(self) -> None:
        """Persona-instance rows (id-keyed) and the identity map."""

        self.data["persona_instance_runtime"] = {"enabled": True}
        ...
        self.data["identity_map"] = identity_aliases_for_rows(self.persona_instance_rows)
        self.data["persona_instances"] = _keyed(self.persona_instance_rows, "persona_instance_id")

    def persona_chat(self) -> None:
        """Chat history (recency pointers), trace and operator channels, timed as ``persona_chat``."""

        self.history_accountant = ProjectionAccountant("persona_chat_history")
        self.trace_accountant = ProjectionAccountant("persona_chat_trace")
        ...
        # anchors stay.
        self.persona_chat_started = time.perf_counter()
        self.omitted_history_session_ids: set[str] = set()
        self.persona_chat_history_full = persona_chat_history_summary(
            persona_instances=self.persona_instances,
            session_db=self.session_db,
            message_tail=DEFAULT_PERSONA_CHAT_MESSAGE_TAIL,
            accountant=self.history_accountant,
            omitted_session_ids=self.omitted_history_session_ids,
        )
        self.data["persona_chat_history"] = _persona_chat_history_frame(self.persona_chat_history_full)
        self.data["persona_chat_trace"] = persona_chat_trace_summary(
            persona_instances=self.persona_instances,
            event_log=self.event_log,
            message_tail=DEFAULT_PERSONA_CHAT_MESSAGE_TAIL,
            accountant=self.trace_accountant,
        )
        self.conversation_accountant = ProjectionAccountant("operator_conversation")
        self.live_operator_channels = operator_channel_summary(
            persona_instances=self.persona_instances,
            persona_chat_history=self.persona_chat_history_full,
            persona_chat_trace=self.data["persona_chat_trace"],
            accountant=self.conversation_accountant,
            intentionally_omitted_history_session_ids=self.omitted_history_session_ids,
        )
        self.sections_ms["persona_chat"] = int(
            max(0.0, (time.perf_counter() - self.persona_chat_started)) * 1000
        )
        ...
        self.data["operator_channels"] = _keyed(self.live_operator_channels, "channel_id")

    def parity(self) -> None:
        """Completeness, drops, the stable section-key set, and the parity envelope."""
        ...
        self.completeness = {
            "persona_chat_history": self.history_accountant.summary(),
            "persona_chat_trace": self.trace_accountant.summary(),
            "operator_conversation": self.conversation_accountant.summary(),
            "running_work": self.running_work_accountant.summary(),
        }
        self.drop_samples = (
            self.history_accountant.drop_samples()
            + self.trace_accountant.drop_samples()
            + self.conversation_accountant.drop_samples()
            + self.running_work_accountant.drop_samples()
        )
        ...
        for _section_key in REPORTED_SECTIONS:
            self.sections_ms.setdefault(_section_key, 0)
        self.parity_started = time.perf_counter()
        self.data["parity"] = parity_envelope(
            self.data,
            build_started=self.build_started,
            build_start_position=self.build_start_position,
            last_event=self.recent_events[-1] if self.recent_events else None,
            recent_events=self.recent_events,
            completeness=self.completeness,
            drop_samples=self.drop_samples,
            sections_ms=self.sections_ms,
        )
        # ``sections_ms`` is stored by reference in the envelope, so recording the
...
#: ``sections_ms`` keys a consumer can rely on are :data:`REPORTED_SECTIONS` —
#: the timed phases — and ``parity`` records its own span after the envelope.
SECTIONS: tuple[tuple[str, Callable[[SnapshotFrameBuild], None]], ...] = (
    ("read_events", SnapshotFrameBuild.read_events),
    ("resolve_scope", SnapshotFrameBuild.resolve_scope),
    ("agents_readiness", SnapshotFrameBuild.agents_readiness),
    ("persona_roster", SnapshotFrameBuild.persona_roster),
    ("prompt_observability", SnapshotFrameBuild.prompt_observability),
    ("boards_offices", SnapshotFrameBuild.boards_offices),
    ("running_work", SnapshotFrameBuild.running_work),
    ("frame", SnapshotFrameBuild.frame),
    ("persona_rows", SnapshotFrameBuild.persona_rows),
    ("persona_chat", SnapshotFrameBuild.persona_chat),
    ("parity", SnapshotFrameBuild.parity),
)

#: The ``sections_ms`` keys guaranteed on every frame (set to 0 when a lane is
#: skipped), in the order they are timed.
...
def _build_snapshot_in_runtime_scope(
    ...
    *,
    ...
    return SnapshotFrameBuild(
        agent_store, event_log, prompt_skills_catalogs, session_db=session_db
    ).run()
