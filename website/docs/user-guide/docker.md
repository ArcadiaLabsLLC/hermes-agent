...
**OpenAI-compatible API clients (Open WebUI, LobeChat, `/v1/...`).** These talk to each profile's **API server**, which binds **port 8642 for every profile** (resolved from `API_SERVER_PORT` / `platforms.api_server.extra.port` — there is no auto-allocation and nothing under `config.yaml` sets this port. A `gateway.port` key *does* exist in `config.yaml`, but it belongs to the agent-runtime remote gateway — the serve socket lane — and has no bearing on the API server: setting it will not move port 8642.). If you want a client to reach a *specific* second profile, give that profile a distinct `API_SERVER_PORT` in **its own** `.env`, otherwise its gateway tries to bind 8642 too and conflicts with the default profile:
