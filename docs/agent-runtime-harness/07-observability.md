...
| `harness serve boot timeline: <k=v …>` | `hermes_cli/harness_parts/serve/boot_phases.py:827-831`, line built by `BootTimeline.log_line` (`boot_timeline.py:173-178`) | operator grep; the same block also rides the `ready` frame (hermes_cli/harness_parts/serve/boot_phases.py:742-753) |
...
| finding | then | now |
|---|---|---|
| `serve_rpc.py` baseline `or 0` — an unreadable event log became watermark 0, killing the sink's baseline gate and re-opening the resync↔restart loop | `baseline_offset = int(...) or 0` | typed absence: `baseline_offset = event_offset_of(watermark)` then an explicit `is None` arm — `agent_runtime/serve_rpc/office_read.py:424-425` |
