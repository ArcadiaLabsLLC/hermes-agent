...
## 14. D7h — the refusal that closed a stream and left no record (R-D26)

Branch `feat/d7-subscribe-denied-log`, worktree `X:/wt/d1-dialable`, from
`7f968847a8` (D6h's docs commit). Commits `f06b0a5c5b` (the line and its six
call sites) and `77179d7666` (the canon re-anchor). Nothing here was run
against `X:/Eternia/.hermes`.

### 13.1 The measured gap, stated exactly

§8's receipts: `stream_lane_activated lane=hub` at 21:55:32.389, then
`stream_teardown cause=stream_error backlog=1` at 21:55:32.396. Seven
milliseconds. The connector's `subscribe` was answered `subscribe_denied` and
mapped to `hubRefused(<reason>)`, and the reason then existed in exactly one
place: a Dart string in the launcher's memory, which went out with the
connection. On the hermes side the asymmetry was total — a successful attach has
logged `stream_attach op=… purpose=…` since EG-2.1, and all six refusal branches
emitted the frame and logged nothing at all.

So the two machines between them held: a teardown cause (`stream_error`), a
backlog depth (`1`), and no reason. Five candidate denials, no way to narrow
them from disk on either side.

### 13.2 What went in

`agent_runtime/stream.py::log_stream_denied`, beside `log_stream_attach`, same
logger (`agent_runtime.stream`), same `key=value` style, INFO:

```
stream_denied lane=stream reason=already_subscribed connection=conn-7 client=cockpit transport=gateway tier=console at=precheck pid=28344
stream_denied lane=telemetry reason=unsupported_lane connection=stdio client=- transport=stdio tier=- pid=28344
```

`lane` leads because it is what the client ASKED for and is the only field that
can name a lane this serve does not have; `pid` rides last, the additive slot
the three other `pid=`-bearing families use, so a denial joins the builds around
it on one key rather than on wall clocks across a timezone boundary. An absent
value renders `-`, exactly as the attach line's `**fields` do.

`hermes_cli/harness_parts/serve.py::_deny_subscribe(sink, connection, lane,
reason, **extra)` — one function, defined beside `_owner_of`, that logs and then
emits. All six inline `sink.emit({"event": "subscribe_denied", …})` blocks call
it; `grep -n subscribe_denied serve.py` now finds one emit, inside the helper.
That is the point: the ruling asks for a line at every refusal, and six
independent emits is a shape where the seventh forgets.

### 13.3 Three decisions worth naming

**The log goes before the emit.** The refusals worth reconstructing are the ones
on a connection that is already leaving, and a client that never reads its frame
is precisely the case where this serve's own log is the only record. Logging
after the emit would lose exactly the case the line was added for.

**No device id, ever.** `connection` is `conn-<n>` (or `stdio`) — an identifier
that means something to a reader of THIS process's log and nothing to anyone
else. `transport` and `tier` ride beside it because "which door, at which tier"
is the question §8 could not answer: a denial on the gateway lane is another
machine's cockpit, one on the loopback lane is this machine's. Neither names the
device. This is one field wider than `stream_attach`, which carries `client` and
the accepted fold set and no lane identity at all; the widening is deliberate
and is the field gap.

**`at=` splits the two branches that share `already_subscribed`.** The
pre-check arm means the key was attached before this frame arrived (a
resubscribe, or a subscription an earlier generation never released); the
`hub_join` arm means this subscribe lost a race to a concurrent one. Different
diagnoses, and the FRAME cannot tell them apart — the launcher switches on
`reason` and that contract is fixed — so the log is the only place the
difference can live. For §8's own symptom (a fresh generation subscribing on a
connection the previous generation used) the two arms are the fork the next run
will land on.

`lane` is the only value on this line taken straight off the wire, so it is the
only one fenced: whitespace is collapsed to `-` and the value clipped to 64
characters. A client sending `{"lane": "stream reason=ok"}` would otherwise
write its own fields into the operator's log, and this line's whole value is
that it can be trusted.

### 13.4 What the frame does NOT do

Nothing. The three keys, their order and their values are unchanged at all six
sites, which is what `test_serve_stream_lane_parity.py` and the two socket-lane
denial tests already pinned and still pin. The launcher's connector switches on
`event`/`reason`; an instrument that moved a key would close the very lane it
was added to explain.

### 13.5 Tests

`tests/agent_runtime/test_serve_socket_lane.py`, one function (+ a
`_denied_lines` parser anchored on the `stream_denied ` prefix, not the shared
`stream_` stem — matching loosely would count an attachment as a refusal, the
exact conflation this line ends):

| assertion | what would break without it |
| --- | --- |
| the two frames, whole and byte-for-byte | an instrument that renamed or reordered a key, i.e. the connector's `hubRefused` switch |
| `reason` in order: `unsupported_lane`, `already_subscribed` | one line emitted for both, or a hardcoded reason |
| `lane`: `telemetry`, `stream` | the lane the CLIENT asked for being replaced by the lane the serve has |
| `connection` equal to the `subscribed` ack's own key, on both lines | a reason with no attribution — worthless on a serve with several clients attached, which is every serve in the field |
| `client`, `transport`, `tier` (`-` on loopback) | the two fields that answer "which door" silently dropping |
| `pid` equal to this process's | the join key going missing from the family that has carried it since BO-3 |
| level is INFO | the line landing at DEBUG, where an operator's log does not have it |

Both refusals are driven over one real socket, in order, on a real
`serve_loop` — the lane check runs before the hub check, so an
already-attached connection asking for a lane this serve does not have is still
refused BY LANE, and the frame and the line have to agree about which of the two
happened.

`already_subscribed` here is the pre-check arm. The `hub_join` arm is a lost
race between two concurrent subscribes for one key and is not reachable from a
single-threaded client; it is covered by the emit being single-homed, not by a
test of its own, and that is stated rather than pretended.
...
bash scripts/run_tests.sh tests/agent_runtime/test_serve_socket_lane.py \
  ...
  tests/agent_runtime/test_serve_stream_lane_parity.py --file-timeout 900
  -> 3 files, 116 passed, 0 failed  (115 before this stage; socket lane 68 -> 69)
bash scripts/run_tests.sh tests/agent_runtime/test_serve_stream_resume_lane.py \
  tests/agent_runtime/test_snapshot_build_logging.py \
  tests/agent_runtime/test_correlation_id.py --file-timeout 900
  -> 3 files, 41 passed, 0 failed   (the other three suites that read this
     module's log lines or the subscribe refusals)
bash scripts/run_tests.sh tests/test_line_endings.py --file-timeout 900
  -> 3 passed  (the canon edits went through Python; run because they did)
ruff check agent_runtime/stream.py hermes_cli/harness_parts/serve.py \
  tests/agent_runtime/test_serve_socket_lane.py   -> All checks passed
python scripts/doc_cite_adjacency.py --exclude archive --exclude planned
  -> passed (baseline capped, nothing new, nothing stale)
```

The cite gate is run in its RULED scope (`AGENTS.md` §"doc-cite adjacency").
It went red on 13 cites the moment the code landed, which is the first time on
this plan that a stage's insertions were INSIDE two heavily-cited files.
`log_stream_denied` shifted `agent_runtime/stream.py` by +47 from line 352;
`_deny_subscribe` shifted `hermes_cli/harness_parts/serve.py` by +63 from 3213,
decaying to +39 past the six collapsed emits. Nineteen live cites were
re-anchored across six canon docs (`77179d7666`) — more than the 13 the probe
flagged, because five more had shifted into text the probe could not check and
would have rotted silently.

Three WAIVED keys were renumbered rather than re-anchored. Each is the probe's
own blind spot, already read and confirmed correct at its line (a paired cite, a
docstring range, a backtick span over ordinary English), so a waiver follows its
cite: 73 keys before, 73 after, nothing added and nothing deleted.
`07-observability.md`'s log-family table grew the `stream_denied` row, because
that table is the census a reader greps a serve child's log against and a family
absent from it is a family nobody looks for.

### 13.7 Owed after this

* **This stage explains a failure; it does not fix one.** The Mac's serve has
  to be REBUILT for the line to exist there, and D3 run #4's switch is what
  produces it. Expect one `stream_denied` on the Mac at the moment the Windows
  cockpit's hub lane dies, naming which of the five it is and which connection.
* If the reason turns out to be `already_subscribed at=precheck`, the defect is
  a subscription the previous bridge generation never released on a connection
  the new one reuses — a D7l lifecycle question, not a serve one. `at=hub_join`
  would mean two generations subscribing at once. Any other reason points at
  the connector's own subscribe frame.
