...
agent_runtime/gateway_endpoints/      the address & dial POLICY, moved DOWN out of the CLI (nothing here imports hermes_cli)
  ...
  listen.py        stores   gateway_listen_config (moved from serve/gateway_listener), listener_endpoint + SOURCE_* (~70)   entry: listener_endpoint
  routes.py        stores   the per-platform default-route probes and their parsers (ROUTE_PROBES after the CHANGE) (~150)
  addresses.py     policy   ipv4/rfc1918/ranking, machine_addresses (interface enumeration) (~130)
  classify.py      policy   on-link tests and classify_dial_error (DIAL_LOCAL_POLICY / DIAL_UNREACHABLE) (~90)   entry: classify_dial_error
  candidates.py    policy   candidate_endpoints (the list a hello advertises), dial_host (~60)   entry: candidate_endpoints, dial_host
...
Nothing is drawn above 300. Layers point down: `gateway_endpoints.listen` ← `routes`/`addresses` ← `classify` ← `candidates`; `gateway_commands.refusals` ← `join_payload` ← the four verb modules.
...
| 245–283 | `_endpoint` 245 — three sources (`live` sidecar / `config` / `unknown`) | `gateway_endpoints/listen.py` as `listener_endpoint(store_root)`; its `gateway_listen_config` dependency moves with it — §1a | stores |
...
| 964–1085 | `DIAL_LOCAL_POLICY` 964, `DIAL_UNREACHABLE`, `_WSAEHOSTUNREACH`, `_HOST_UNREACHABLE_ERRNOS`, `_V6_*`, `_in_network` 989, `_shares_64`, `_is_on_link` 1013, `classify_dial_error` 1045, `LOCAL_POLICY_SENTENCE` 1081 | `gateway_endpoints/classify.py` | policy |
| 1088–1160, 1202–1211 | `_candidate_endpoints` 1088, `_dial_host` 1129, `_self_endpoints` 1202 | `gateway_endpoints/candidates.py` | policy |
