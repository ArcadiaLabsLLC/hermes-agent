...
**RULED 2026-09-25 (owner, as recommended):** Q6 yes — a positive control lands in the MOVE before any table replaces an untested routing site; Q7 yes — door now, held widening row per private name; Q8 yes — a store class moved whole may cross 800 for exactly one MOVE commit (the H4 precedent), the next commit splits it. The three questions as the sheets raised them, kept for the record: (Q6) three ladder/routing sites have **no test reaching them today** — `persona_assignments._persona_instance_is_active_lane`, `serve_socket._os_error_token`, `realm_sync.sync_artifacts_for_workspace_agent`'s caller — so their killing mutations are green by construction; the sheets land a positive control FIRST, in the MOVE (default: yes, a control before the table — the capture-is-a-vehicle rule). (Q7) `prompt_observability` and `profile_runner` reach three PRIVATE upstream names (`_compression_threshold_for_model`, `_find_all_skills`, `_sanitize_surrogates`); the sheets route each through `agent_runtime/_upstream_doors.py` and file a held widening row per name in `upstream-footprint-ledger.md` (default: door now, widening PR when the next upstream batch is cut).

### 3.1c Wave 2 — the second batch (twelve sheets, 2026-09-25, lane S2A against `28012c8f8a`)

Sheets in [`god-file-layout-sheets/`](god-file-layout-sheets/): `pipeline.md`, `draft.md`, `agent_chat_tool.md` (C1/T1 — fork-only files in upstream directories, ruling Q2), `gateway_commands.md`, `gateway_peers.md`, `agent_create.md`, `persona_chat_continuity.md`, `dispatch_delivery.md`, `operator_channels.md`, `running_work.md`, `stream.md`, `mcp_admission.md`. Every sheet applies §3.1b's two facts (package named after the file; a class moved whole for exactly one commit, Q8) and the three batch-1 decision rules where they bite: a fork-wide vocabulary word is read by NAME from a constant in its single reader and never enum-ised (W0-G5 arm (c) counts every `StrEnum`/`Final` member fork-wide — `running`, `completed`, `failed`, `done`, `blocked`, `unknown`, `ok`, `expired`, `dropped` are all `states.py`/`turn_visibility`/`RpcRefusal` members, and six of these twelve files compare against them for a DIFFERENT question; the gate defect is one class row, filed with the report); a class moved whole may cross 800 for its one MOVE commit (`CharacterDraft` 1,101, `PersonaChatClarifyTicketStore` ~515); where a layout would create a cycle the sheet names the module that breaks it (`gateway_peers/cache.py` lazy; `dispatch_delivery/completions.py` owns the root resolvers; `persona_chat_continuity/lease.py` imports no applier).

**Three owner rulings of 2026-09-25 shaped every sheet in this batch, and the sheets say where each bit.** (1) Each sheet's §1 opens with a SKELETON — the package tree, one line per module with its purpose, layer and entry points, and a per-entry-point table of the modules an agent opens to follow it — and that text is the package `__init__` map verbatim; the line-range table is §1.1. (2) Modules TARGET 100–300 code lines, hard cap 500; 800 is only the gate's definition of a god file. Sheets that refolded under this: `gateway_commands.md` (the address policy, first drawn as one ~480 module, became `agent_runtime/gateway_endpoints/`), `pipeline.md`, `draft.md`. (3) No fragmentation: a new module has a FLOOR of 100 code lines unless it is a vocabulary, errors or table module (each sheet names its exempt module); two sub-100 modules that only make sense together are one; no flow — one verb, one request, one write — spans more than three modules, and the per-entry table makes the count visible. Sheets that refolded under this: `pipeline.md` (`compose` absorbs `validate`), `draft.md` (`layout` absorbs locations and codec; `_support` is the one stated sub-100 exception), `gateway_commands.md` (five runtime modules → three), `gateway_peers.md` (`proofs` into `trust_store`), `persona_chat_continuity.md` (the scopes into `lease`), `dispatch_delivery.md` (provenance into `forge`), `operator_channels.md` (relationships into `instances`, the coercions into `vocabulary`), `running_work.md` (five lane modules → two), `stream.md` (deferral + receipts → `build_policy`; the fingerprint into `session`), `mcp_admission.md` (teardown into `registration`, the render line into `outcomes`), `agent_create.md` (persona into `request`; placement/skills/reply into `phases`), `agent_chat_tool.md` (log_path into `threads`). Where a module sits above 300 after the refold the sheet says why (one write door with its writers beside it; the three-module bound outranks the 300 target).

**The base, hashed** (`git show 28012c8f8a:<file> | sha256sum`; raw = `wc -l`, code = the §0.1 counter):

| file | raw | code | sha256 @ `28012c8f8a` |
|---|--:|--:|---|
| `agent/charsheet/pipeline.py` | 2,533 | 1,898 | `74c1fd3b3e7f5738a895549efd5e7b9ba338a8dfac794c171929deb9aa8a32ea` |
| `hermes_cli/harness_parts/gateway_commands.py` | 2,323 | 1,645 | `c04c4cdf93db733fba5cba2f27dcfa2d3f6d41a1f71c2e3d7f4124da1f404e67` |
| `agent_runtime/persona_chat_continuity.py` | 2,240 | 1,646 | `7a4f94b5c33afb1b55d63f93e4f7b16644b3f1d2faad6a20a6b812b343f36e0c` |
| `agent_runtime/gateway_peers.py` | 2,185 | 1,621 | `90020d187da5e3b7d0c56c4d268ab6ed2a74961e8c0fd0b1f6a508d47d41f948` |
| `agent_runtime/agent_create.py` | 2,126 | 1,439 | `33a970791b01a0bfb38cfa64f1ec84d17a786ef42a08167accfe06ddd28ce2ca` |
| `agent_runtime/mcp_admission.py` | 2,073 | 1,452 | `e1037484dc09d04cf2e29372f6f2140bc771c185e3a6fc1cefd64a528e577b68` |
| `agent/charsheet/draft.py` | 2,000 | 1,600 | `59035684b4ac243045d5acf36ae28da096bd59d9493a00dce78ede3f446dd1d4` |
| `agent_runtime/running_work.py` | 1,964 | 1,517 | `ad55a7c70b87fed9db780643b7a1ad1be5652009c0dcc9035c34d85f0888fae9` |
| `agent_runtime/stream.py` | 1,898 | 1,385 | `7b2049237cb8b395c4c09540c87ad67b7a0a73ec7f035292afe624ef36f2fcdf` |
| `tools/agent_chat_tool.py` | 1,898 | 1,429 | `d5fa1710db6b5c27f0e15bc075549dfb6d1ff05fc8f102641e77df69ead8d268` |
| `agent_runtime/dispatch_delivery.py` | 1,801 | 1,311 | `7a654ba97eb00ff70e8e0e811d9408d7fd5be1206682127ce4466ba888802955` |
| `agent_runtime/operator_channels.py` | 1,670 | 1,394 | `5ed656c3260ab046b0177f276d9bbe8330ed84c5ebf63f9aec0d3fe6d19d3c42` |
| **TOTAL 12** | **24,711** | **17,937** | |

**Exec lanes, grouped by RAW lines** (owner ruling 2026-09-25: lane size follows file size, never file count; ~5–7 k raw per lane). Inside a lane the files are sequential, in the order written, so a file lands before the file that imports it:

| lane | files, in order | raw | why this order |
|---|---|---:|---|
| **B1** (C1 + T1) | `pipeline` (2,533) → `draft` (2,000) → `agent_chat_tool` (1,898) | **6,431** | `draft` imports `pipeline` as a module and the `_upstream_doors` it creates; `agent_chat_tool` is disjoint and rides for size |
| **B2** (R4) | `gateway_peers` (2,185) → `gateway_commands` (2,323) → `agent_create` (2,126) | **6,634** | `gateway_peers` creates `cache.unusable_reason`; `gateway_commands`' MOVE deletes the upward import `gateway_peers/dial.py` still carries; `agent_create` is disjoint |
| **B3** (R1 + R2) | `persona_chat_continuity` (2,240) → `dispatch_delivery` (1,801) → `operator_channels` (1,670) | **5,711** | `dispatch_delivery` imports two `persona_chat_continuity` names at module level; `operator_channels` is disjoint and creates `clock.parse_iso` |
| **B4** (R2 + R3) | `running_work` (1,964) → `stream` (1,898) → `mcp_admission` (2,073) | **5,935** | `stream` reaches `running_work_store_paths`; `mcp_admission` is disjoint and adds eight doors to `_upstream_doors.py` |

**The exec-lane contract (owner ruling 2026-09-25, the second of the day).** One MOVE commit and one CHANGE commit per file — no fix-up commits; a sheet's dead-code deletions are NOT a third commit (they land under the dead-code queue's "Working a slice", by verdict rows the CHANGE closes). Touched tests once per file (the MOVE's killing mutation and the CHANGE's mutations, run and pasted), no tooling gates per commit: the orchestrator runs the gates once per landed batch of lanes, not per landing. The MOVE's hash proof is §3.3's, unchanged: one sha256 row per §1.1 span, pasted in the commit body.

**Cross-lane one-liners, each with "tree wins" (the second lane to arrive folds):** `clock.parse_iso` (B3 `operator_channels` creates it; B4 `running_work` folds); `serde.is_hex` (B1 `agent_chat_tool` / B2 `gateway_peers`); the Q10 door's binding line in the plugin `__init__` and `serve/session.py` (B3 `dispatch_delivery` / B1 `agent_chat_tool`); `redaction.scrub_tree` (B4 `stream` creates it; B3 `persona_chat_continuity` folds `_redacted` later); `file_locks.try_lock_fd/unlock_fd` (B3, with a two-line retarget in `mission_chat_turns.py`); `serve_gateway_auth`'s three pairing names made public (B2); the `_upstream_doors.py` additions (B4 twice, in one lane); `gateway_listen_config` re-exported from `serve/gateway_listener.py` (B2). Every other cross-file edit in the twelve sheets is a retarget of an import line, listed per sheet in its §8.

**What the batch found that is not a sheet's to fix** (filed with the report, per the fork `CLAUDE.md` weakness rule): the W0-G5 vocabulary-arm class defect (six files compare against `states.py` words for a different question — a gate row in `fork-hygiene-queue.md`); `agent/charsheet/draft.py:77`'s module-level `agent_runtime` import against the package's own stated packaging boundary (Q9; a runtime-queue row); the two callers of the private CLI handler `chat_turn_message._cmd_mission_chat_message` from outside the CLI (Q10; a runtime-queue row, joining the "reaches UP" row the W0 lane filed).
### 3.1d Wave 2 — batch two, the twelve smaller files (2026-09-25, lane S2B against `28012c8f8a`; S2A's twelve larger files are §3.1c)

Sheets in [`god-file-layout-sheets/`](god-file-layout-sheets/): `state_patches.md`, `store.md`, `board_store.md`, `dispatch_store.md`, `mission_chat_turns.md`, `runtime_hud.md`, `chat_live_log.md`, `terminal_envelope.md`, `config.md`, `harness_doctor.md`, `persona_instance_sync.md`, `agent_chat_dispatch.md`. Every sheet is written to the three owner rulings of 2026-09-25 and states where each bites: **(1)** §1 opens with a SKELETON — the package tree, one line of purpose per module, its layer, and per entry point the modules an agent must open to follow it — and that text becomes the package `__init__` map; **(2)** a new module targets 100–300 code lines with a FLOOR of 100 (vocabulary / table modules exempt, and each sheet names which) and a hard cap of 500 — 800 is only the gate that defines a god file; two sub-100 modules that only make sense together are one; **(3)** no flow — one verb, one request, one write — spans more than three modules; a skeleton that drew one was refolded and the sheet says so. Under (2)–(3) the first drafts of eight sheets were refolded (32 modules → 21 across `state_patches`, `store`, `board_store`, `dispatch_store`, `mission_chat_turns`, `runtime_hud`, `chat_live_log`, `terminal_envelope`); the other four were written to the rulings directly. The batch-1 facts stand: every package is named after its file; a class moved whole may cross the cap for exactly one MOVE commit (Q8 — `board_store`'s `BoardStore`, 900 raw); a vocabulary word spelled fork-wide is named as a constant in its single reader and never enum-ized (the W0-G5 arm-(c) defect, fork-hygiene row 2026-09-25); where a layout would create an import cycle the sheet names the module that breaks it.

**Exec lanes, grouped by RAW lines (owner ruling 2026-09-25: lane size follows file size, never file count), ~5k raw each, 15,304 raw in all.** Inside a lane the files are sequential in the order given — a file lands before the files that import it — and the helper owners a lane creates are folded toward by every later sheet ("tree wins" when two lanes race to create one).

| lane | files, in order (raw) | raw total | why this order |
|---|---|---|---|
| **2B-A** (R1 stores) | `state_patches` 1,439 → `store` 1,114 → `board_store` 1,085 → `dispatch_store` 1,308 | **4,946** | `store` lazily imports `state_patches`; `store.WorkspaceStore.delete` lazily imports `BoardStore`; `dispatch_store` is disjoint and last |
| **2B-B** (R2 chat lane) | `mission_chat_turns` 1,613 → `runtime_hud` 1,437 → `chat_live_log` 1,066 → `terminal_envelope` 1,262 | **5,378** | no sibling imports `mission_chat_turns`; `runtime_hud` imports three `terminal_envelope` names (through `__init__` once it lands); `chat_live_log` is disjoint |
| **2B-C** (R3/R4/T1) | `config` 1,212 → `harness_doctor` 1,262 → `persona_instance_sync` 1,357 → `agent_chat_dispatch` 1,149 | **4,980** | `harness_doctor` imports `config` (and the `OVERRIDE_STATE_*` constants its CHANGE creates); the other two are disjoint; `agent_chat_dispatch` is the one file outside `agent_runtime/` (Q21) |

Base-file sha256 at `28012c8f8a` (the MOVE hash proof's left-hand side; `git show 28012c8f8a:<file> | sha256sum`):

| file | sha256 |
|---|---|
| `agent_runtime/state_patches.py` | `2a121504d135d92254f60743db28d678247ee553ba9f95eaf07b7cb552f5bdd3` |
| `agent_runtime/store.py` | `70e42ce25af8bcfebc8f0896a8c63c0b3e4826a83c67cc56aa7e249ec182f305` |
| `agent_runtime/board_store.py` | `3f4969014ac5489d4911d1ebfc810560692571ef07fb4a1bab33750434fa52bf` |
| `agent_runtime/dispatch_store.py` | `9d820992700c6f5307088751b50e0e85873ba42b14bf5c13bf26f225370b7163` |
| `agent_runtime/mission_chat_turns.py` | `b53577ec4971034c4d641a5616f89ab8a8a484cf660fab4f779522cf06173a8f` |
| `agent_runtime/runtime_hud.py` | `55ffa9d51f6a25fb077beed33765b13a7e9719231156459e744ed96ae9ba4537` |
| `agent_runtime/chat_live_log.py` | `77f64791321ea1f01dde09abf4d762db7c213a8adc65f0416dc3cc9590b84707` |
| `agent_runtime/terminal_envelope.py` | `ea8b97b119dddec8b434f88fa2d04795d3293870b9e1611bf3344ff1d7b9807a` |
| `agent_runtime/config.py` | `0112a37640925c1ea740aec993c9dd425b05c158787c121492b7bfeda35c0682` |
| `agent_runtime/harness_doctor.py` | `993f04c49b9bb0a5bfd47f27f2ca0c4ba39cc957fa01cf2f52885785c2634da7` |
| `agent_runtime/persona_instance_sync.py` | `5376425b358f991c7f1503da43e64d96d1d9841167c506903fb6282149e054cb` |
| `tools/agent_chat_dispatch.py` | `c65ebf5e2bb2541eee75c287291e5e31f5de4c9545fe905c3d833d2318401007` |

**The exec-lane contract (owner ruling 2026-09-25).** Per file exactly two commits — one MOVE (spans byte-identical, the sha256 table in the body, `__init__` re-exports, seams retargeted, the sheet's positive controls landed, `__layer__` declared) and one CHANGE (the sheet's §2–§4 with each killing mutation's red pasted) — and no fix-up commits: a defect found after the CHANGE is a row, or the next file. A lane runs the touched tests once per file and NO tooling gate per commit; the orchestrator runs the gates once per landed batch. Findings file on arrival into the domain queue; a lane that cannot write the queue puts the row verbatim in its report.

**Helper owners this batch creates, and who folds toward them** (each named in the sheet that creates it; "tree wins" on a race): `events.payload_bytes` (2B-A/state_patches); `serde.bounded_text` (2B-A/dispatch_store; `chat_turn`, `mission_chat_outcome`, `persona_open_chat` fold in their lanes); `serde.number_or_bounded_text` and `clock.now_iso_micro` (2B-B/mission_chat_turns; `mission_chat_phases` in the same commit, `persona_chat_continuity` in its lane); `clock.parse_iso_utc` (2B-B/runtime_hud or 2B-A/store — `store.ledger_time` becomes its alias); `paths.path_exists_safe` and `redaction.mask_secret_lines` (2B-B/chat_live_log; `persona_chat_history`'s two copies fold in a one-line follow-up); `personas.persona_id_aliases` and `personas.role_or_attr` (2B-B/terminal_envelope; `config`'s three alias copies fold in 2B-C); `serde.optional_str` (2B-C/config; `mission_chat_outcome` folds in its lane); `store_conflicts.park_conflict_sidecar` (2B-C/persona_instance_sync; `flow_graph_sync`, `level_sync`, `map_sync` fold in their lanes); `agent_runtime/subprocess_pumps` (2B-C/agent_chat_dispatch or R2's `persona_chat_actor_prewarm`); `_upstream_doors.terminate_host_pid` + its held widening row (2B-C/agent_chat_dispatch). Candidates the sheets name but do not require: `errors.error_text`, `process_identity.start_ticks`, `store_file_io.append_jsonl`, `realm_sync/sidecar.read_baseline/write_baseline`.

**Owner questions raised by this batch** are §9 Q20–Q22.

### 3.1e Wave 2 — batch three, the three held files as ONE exec lane (2026-09-25, lane S3 against `8411d26a01`)

Sheets in [`god-file-layout-sheets/`](god-file-layout-sheets/): `changed_line_mutation_check.md` (S1), `id_markers.md` and `hermes_cli_conftest.md` (T2). These are the last three files of the 62 that had no sheet: test infrastructure and a script, so by the scope rule their packages go under `scripts/<pkg>/` and `tests/_downstream/<pkg>/` — `scripts/mutation_check/` (the entry file keeps its path: CI, the ps1, `CLAUDE.md` and nine test files name it), `tests/_downstream/id_markers/` and `tests/_downstream/hermes_cli_conftest/` (both named after their file, because the import STRING is the contract: the root `conftest.py` registers one by name, `conftest_plugin.py` re-exports the other's hooks). Every sheet is written to the three owner rulings of 2026-09-25 (skeleton first, 100–300 code lines with the floor and the cap, no flow over three modules) and applies the batch-1 facts; where each bites: `changed_line_mutation_check.md` refolded §3.2's `{claims,partition,run,report}` into `{schema,anchor,selection,run}` (a `report` module would be forty lines of print blocks, a `claims` module fifty); `id_markers.md` splits the 314-row table by the CLASS that RETIRES a row — fork behaviour, a POSIX premise, upstream's own Windows red, an optional distribution — never by size and never by the lane that filed it, and names `distributions.py` (~30 code) as its one table-module floor exemption; `hermes_cli_conftest.md` folds §3.2's five families into four modules plus a table (`fences`, `isolation`, `probes`, `registry`, `hooks`), the web-prereq probes joining the registry probes because both answer once per process. Three things the three sheets share: a package `__init__` in `tests/_downstream/` uses ABSOLUTE imports (one gate execs the file under a synthetic module name); W0-G6 does not walk `tests/_downstream/` or `scripts/` today, so every `__layer__` these packages declare is bound by import and judged by nobody until Q31; and the MOVE hash proof for `id_markers` is per ROW, not per span, because the base file's nineteen blocks interleave the classes.

**The base, hashed** (`git show 8411d26a01:<file> | sha256sum`; raw = `wc -l`, code = the §0.1 counter):

| file | raw | code | sha256 @ `8411d26a01` |
|---|--:|--:|---|
| `tests/_downstream/hermes_cli_conftest.py` | 1,347 | 893 | `0acfe569532eafedb3981d538524d51c39da6db16c27d6259a7c1406316c1ebe` |
| `tests/_downstream/id_markers.py` | 1,216 | 1,053 | `5aed29d0fb6ded233bac105e647e2658f9a3b2d0928be490b398031a4ae60145` |
| `scripts/changed_line_mutation_check.py` | 1,150 | 831 | `5f1a9227586b6756421e93ccca87043bb6130f1a2ce120c88ec420f7c5aa5139` |
| **TOTAL 3** | **3,713** | **2,777** | |

**One exec lane** (owner ruling 2026-09-25: lane size follows file size; 3,713 raw is under the ~5 k a lane carries, so the three ride together). Inside the lane the files are sequential, and the order is the import order:

| lane | files, in order | raw | why this order |
|---|---|---:|---|
| **B5** (S1 + T2) | `changed_line_mutation_check` (1,150) → `id_markers` (1,216) → `hermes_cli_conftest` (1,347) | **3,713** | the gate lands first because every later landing runs it — this lane's own four later commits included — and its six self-anchored claims make its MOVE the one the gate mutates for free; `hermes_cli_conftest/registry.py` imports `id_markers.reasons` through the `id_markers` package, so `id_markers` lands before its importer |

**The exec-lane contract (owner ruling 2026-09-25, restated once).** Per file exactly two commits — one MOVE (spans byte-identical with the sha256 table in the body; for `id_markers` one row per key, 314 rows; `__init__` re-exports; the monkeypatch and loader seams retargeted; the sheet's positive controls landed; `__layer__` declared; the grandfathered fixture rows re-keyed by hand, since `--write-fixtures` refuses a re-key as NEW) and one CHANGE (the sheet's §2–§3 with each killing mutation's red pasted) — and no fix-up commits: a defect found after the CHANGE is a row, or the next file. The lane runs the touched tests once per file (for the gate: the nine `tests/scripts` files and `test_mutation_gate_worktree_lock.py`; for the two test-infrastructure files: the `*_downstream.py` tests that pin them plus `tests/test_env_gap_registry.py`, `tests/test_claude_code_credentials_file_gate.py` and `tests/tooling/test_downstream_size_ceiling.py`) and NO tooling gate per commit; the orchestrator runs the gates once for the landed lane. The gate's CLI contract is proven by a DUMP, not a belief: `--list --base <B>`, `--claims-for`, `--help` captured before and after each of its two commits, `cmp` exit 0, with `<B>` chosen so every line class appears and named in the body.

**Cross-lane one-liners, each with "tree wins" (the second lane to arrive folds):** `tests/_env_gap_fence.KnownDefectTracker` (B5 `hermes_cli_conftest` creates it with the failed-OR-`wasxfail`, owner-prefixed semantics; `tools_conftest` folds in its own lane WITH the xfail control, because its copy matches `failed` only); the `<dir>_conftest/__init__.py` arm of `tests/test_env_gap_registry._registry_module_path` (B5 creates it; the other two conftests inherit it the day they become packages); the mark-NAME constants in `id_markers/reasons.py` (`conftest_plugin` imports them for `addinivalue_line` and its consumer fixtures; any lane that adds a mark declares it there, never as `pytest.mark.<word>`); `scripts/mutation_check/selection._git` is package-local and never `god_file_probe._git` (the probe is a subject of the gate); the six self-claims in `tests/mutation_claims.json` re-point in the gate's MOVE and nowhere else. Every other cross-file edit in the three sheets is a retarget of a monkeypatch site or an import line, listed per sheet in its §6.4 / §8.

**What the batch found that is not a sheet's to fix** (filed with the report, per the fork `CLAUDE.md` weakness rule): `tests/_downstream/tools_conftest.py`'s known-defect lane still runs the pre-ML-16 classifier (`failed` only, no owner-prefix filter — a strict xfail there would retire its own banner; a fork-hygiene row); `tests/test_env_gap_registry.py` reads a fence only as a flat FILE, so packaging a fence reproduces the "green by blindness" its own docstring records (the arm lands in B5's MOVE; a fork-hygiene row until it does); the mark-only `_ENV_GAPS` lane is empty in all three downstream conftests and asserted empty by the registry gate — three dead registries and the shared `apply_marks` / `StaleEntryTracker` / `register_marks` behind them are one CLASS row in the dead-code queue (Q30); the `ids_marked` dead-code row is REFUTED (two gates read it); `HUNK` in the gate and `_HUNK_RE` in `scripts/check_profile_scope_patterns.py` are one regex spelled twice (S2's row).

**Owner questions raised by this batch** are §9 Q30–Q32.
...
| `refusals.refusal` | 5 (`flow_graph_sync`, `level_sync`, `map_sync`, `mission_chat_workdir`, `persona_config_sync`); `gateway_commands._refusal` rendered a harness ERROR, not a report row, and is `gateway_commands.refusals.store_refusal_error` since B2 — `tools/agent_chat_tool` is a seventh if T1 runs | R4 creates |
...
**Q9–Q10 (batch 2, 2026-09-25, lane S2A) — open; each default is what the exec lanes follow unless overruled:**

9. **`agent/charsheet/draft.py:77` imports `agent_runtime.profile_home.get_shared_characters_dir` at module level, and the package says it must not.** `agent/charsheet/__init__.py` ("nothing here imports `agent_runtime`"), `revisions.py:44` and `draft_lock.py`'s docstring all state the packaging boundary — `agent_runtime` is not in the shipped wheel, which is why the lock was built inside the package — and this one line breaks it for every plain-wheel import of `agent.charsheet.draft`. Recommend **keep the reach, isolate it**: one lazy function in `agent/charsheet/_support.py` (`shared_characters_dir()`), called from `layout.characters_dir` at call time so the module imports cleanly and only the call needs the runtime, and correct the three docstrings to say so. The alternative — retiring the boundary and moving `agent/charsheet/` under `agent_runtime/` — is a program-scale move, not a lane's, and would make the charsheet skill's wheel story false; it is the owner's call, not the default.
10. **Two callers outside the CLI run a mission-chat turn in-process through the PRIVATE handler `hermes_cli.harness_parts.persona.chat_turn_message._cmd_mission_chat_message`** — `agent_runtime/dispatch_delivery.forge_delivery_turn` (1131) and `tools/agent_chat_tool.agent_chat_send` (579); `agent_chat_tool` also reads `chat_target`'s private decision functions. This is the "runtime reaches UP into the CLI namespace" row W0 filed, with the second instance now named. Recommend **one door, bound at boot**: `agent_runtime/mission_chat_door.py::run_mission_chat_turn(args) -> tuple[int, dict | None]`, a module-level slot the plugin's CLI registration and `serve/session.py`'s boot both bind to the handler (the CLI already registers what the runtime needs — the inversion the row asks for); an unbound door raises a typed `MissionChatDoorUnbound`, never a silent no-op; both callers call the door and the two private imports are deleted in their CHANGE commits. The alternative — making the handler public and keeping the upward import — leaves W0-G6's `harness_imports` arm red for both files by design; not the default.
**Batch-two questions (lane S2B, 2026-09-25; Q9–Q19 are S2A's). Each carries a recommended default the sheets already apply; a lane treats the default as ruled until the owner says otherwise.**

20. **`terminal_envelope`'s hard-floor arms.** Since ruling R-2 `GRANTABLE_COMMAND_CLASSES = COMMAND_CLASSES`, so the `envelope_command_not_grantable` arm of `envelope_decision`, the `envelope_grant_class_not_grantable` issue arm of `resolve_terminal_envelope_grants`, and `hard_floor_command_classes()` are unreachable by construction (dead-code queue row 63 names one; the sheet names its twin). Delete the arms and the accessor, or keep them as the "reserved for a future floor" the constants' comments describe? Recommend **delete the arms and `hard_floor_command_classes`; keep `ENVELOPE_COMMAND_NOT_GRANTABLE` / `GRANT_CLASS_NOT_GRANTABLE` exported with a "no producer since R-2" note**, because the wire vocabulary a launcher may parse should not move in a refactor and dead branches that a future floor would rewrite anyway are not a reservation, they are a liability the legibility floor exists to retire.
21. **Where `tools/agent_chat_dispatch.py`'s package goes.** §0.3 says T1's modules go under "a new `tools/agent_chat/` package"; the batch-1 precedent names a package after its file so no importer moves. Recommend **`tools/agent_chat_dispatch/`** — a fork-only directory replacing a fork-only file of the same name, footprint-neutral in the upstream tree, no shim to delete later; `tools/agent_chat_tool.py` (S2A) lands the same way. The alternative is `tools/agent_chat/dispatch/` plus a ≤ 20-line shim at the old path until every importer is retargeted in one commit.
22. **`chat_live_log`'s `ts` spelling.** Folding `_now_iso` onto `clock.now_iso` (program §4's row) changes every mirror line's timestamp from `hermes_time.now().isoformat()` (`+00:00`, microseconds) to `Z` + milliseconds — a wire-visible change to a greppable file that no reader parses and no test pins. Fold (with the sheet's control pinning the new spelling first), or keep the `+00:00` spelling as this file's own and strike the §4 row? Recommend **fold** — one clock spelling for the fork is the rule, the file is a regenerable artifact by its own contract, and the control turns the stated gap into a pinned format either way.

**Batch-three questions (lane S3, 2026-09-25; the three held files). Each carries a recommended default the sheets already apply; a lane treats the default as ruled until the owner says otherwise.**

30. **The mark-only `_ENV_GAPS` lane.** `_ENV_GAPS = {}` in all three `tests/_downstream/*_conftest.py`, and `tests/test_env_gap_registry.py::test_no_directory_still_uses_the_mark_only_registry` asserts it STAYS empty (an absent dict passes too). Behind the three empty dicts sit `hermes_cli_conftest`'s hand-rolled copy of the lane (its group loop, `_STALE_ENV_GAP_ENTRIES`, the stale arm and the stale terminal section — reach census 0 hits) and, in the two siblings, the shared `_env_gap_fence.apply_marks`, `StaleEntryTracker`, `EnvGapRegistry` and `register_marks` (the two marks nothing applies). Delete the lane fork-wide, or keep it as the shape a future mark-only row would use? Recommend **delete**: B5 deletes the `hermes_cli` copy in its CHANGE (the sheet's §5), and the sibling copies plus the shared helpers are ONE class row in the dead-code queue — a lane the gate itself forbids populating is not a reservation, and the probe-backed `_ENV_GAP_SKIPS` is the only form the registry gate can see.
31. **Should W0-G6 walk `tests/_downstream/` and `scripts/`?** `probe.layered_roots` is `agent_runtime/`, `hermes_cli/harness_parts/`, `plugins/eternia-harness/` plus the fork-only trees under `agent/` and `tools/`; the three packages this batch designs declare `__layer__` that nothing judges. Widen to both roots wholesale (every flat script and conftest becomes an `undeclared` grandfather row — sixty-odd), widen to PACKAGES only, or leave the two roots outside? Recommend **packages only, in B5's landing**: a package the program creates is born declared, the flat files stay outside until their lane opens them, and the fixture gains no rows for code nobody is refactoring.
32. **The three fork-only helpers inside the upstream directory `tests/hermes_cli/`** (`__init__.py`, `_gateway_fence.py`, `_module_identity.py`) — the C1/T1 class of §0.3, imported by nine fork test files and by `hermes_cli_conftest/fences.py` and `isolation.py`. Move them under `tests/_downstream/hermes_cli_conftest/` (the scope rule's "never inside an upstream package") or leave them? Recommend **leave, this batch**: they are the fence's own documented location (`_gateway_fence.py` carries the reproduction the fixtures cite), nine importers would move for no behaviour change, and `tests/hermes_cli/` is a directory upstream ships tests in, not a package upstream owns a module map for; file it as a fork-hygiene row for the seam program's next CARRY pass rather than spend a MOVE commit on it here.
