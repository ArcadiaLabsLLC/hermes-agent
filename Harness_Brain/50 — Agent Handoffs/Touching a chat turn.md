...
- Handler: `hermes_cli/harness_parts/persona_commands.py` (`_cmd_mission_chat_message` 692 lines → `_mission_chat_commit_turn` 1,590; the refactor's H3 lane makes it a `TurnCommit` with one method per phase).
- Runner: `agent_runtime/profile_runner.py::ProfileAgentRunner._execute_agent_run`.
