...
**Verified 2026-09-26 (lane UPPR, fresh `upstream/main` trees at d0288be5b3):** the five rows dated 2026-09-26 above each carry a recorded red on the base tree and a green after the fix; `up/monkeypatch-undo-scoped`, `up/profile-home-generic` (superseded by `5e4a2a3d24`, `dep_ensure.py` gone) and `up/profiles-delete-guard` (psutil pinned on every platform) were re-tried and refuted a second time — the UPREV drop stands. Reviews received 2026-09-26 on #121218 (blocking: the `if level:` skip lets a non-stdlib relative import through; a `__package__`-rooted guard is the two-sided fix) and #121023 (the AST walk the comment claims to skip was already unreachable for bundled plugins; correct the comment or drop the change), both from an automated non-maintainer reviewer; full text `X:/wt/_holds/pr-replies-0926/`.
