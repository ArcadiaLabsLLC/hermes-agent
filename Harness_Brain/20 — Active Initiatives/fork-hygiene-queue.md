...
- [ ] **Seam program, stage the plan** · `fork / upstream` · 22 upstream files carry > 200 fork lines and 246 upstream TEST files carry fork edits (13,700 lines); write the staged plan (house format) that makes every fork touch additive, moves fork tests to fork-only files, upstreams the generic fixes, and adds the two ratchets (upstream files edited + upstream lines deleted, only down) · evidence: [[Fork Boundary Map]] § seams; [[Upstream Sync]] § seam program **UNCLAIMED**
