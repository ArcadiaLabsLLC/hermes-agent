...
| transport | `serve_rpc.py` (`_METHODS` registry, `method()` decorator, tiers), `serve_socket.py` (server / client / `SocketOwnerLock`), `stream.py` (frames, folds, Stage-5 demote deferral), `serve_registry.py`, `gateway_peers/`, `gateway_identity.py`, `gateway_tls.py`, `gateway_announce.py` | [03](../../docs/agent-runtime-harness/03-transport-and-wire.md), [09](../../docs/agent-runtime-harness/09-multi-device-runtime.md) |
...
| chat turn | `mission_chat_turns/` (v3 ledger), `chat_turn.py`, `chat_turn_reservations.py`, `profile_runner.py` (`ProfileAgentRunner`), `mcp_admission.py`, `terminal_envelope.py`, `prompt_observability.py`, `runtime_hud.py`, `persona_chat_history.py`, `persona_chat_continuity.py`, `persona_open_chat.py`, `mission_chat_steer.py`, `chat_live_log.py`, `operator_channels.py`, `running_work.py`, `dispatch_store/`, `dispatch_delivery.py` | [05](../../docs/agent-runtime-harness/05-chat-turn-lane.md) |
| office + board | `office_store.py`, `office_layout_policy.py`, `board_store/`, `level_sync.py`, `flow_graph_sync.py` | [06](../../docs/agent-runtime-harness/06-office-and-board.md) |
