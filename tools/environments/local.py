...
def _windows_bash_candidates(custom: "str | None") -> list[str]:
    ...
    if found and not _is_windows_system_shim(found) and found not in candidates:
        # ``_is_windows_system_shim`` covers the System32 WSL launcher; the Store's
        # WindowsApps bash.exe is the same kind of stub and is skipped here.
        if "/windowsapps/" in os.path.normpath(found).replace("\\", "/").lower():
